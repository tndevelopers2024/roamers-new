...
export function initStory() {
  ...
  const AR = 1672 / 941;                                   // hero-cliff: 16:9
  const SOLO = { u: .885, v: .62 };                        // the traveller on the right, both arms spread
  const GROUP = { u0: .46, v0: .5, u1: .99, v1: .92 };     // all six on the cliff edge, in photo coordinates
  const START = { x: .5, y: .74, mx: .5, my: .9 };          // where SOLO sits on screen before scrolling: between the headline and the text below
  ...
  const renderHero = () => {
    ...
    const s0 = vh / (H * .5);                              // opening zoom; kept gentle so the 1672px photo stays sharp
    ...
    const ax = lerp(vw * (mobile ? START.mx : START.x), (frame.x0 + frame.x1) / 2, t);
    const ay = lerp(vh * (mobile ? START.my : START.y), (frame.y0 + frame.y1) / 2, t);
  ...
  mm.add(DESK, () => {
    ...
    $$('.chapter', track).forEach(ch => {
      ...
      gsap.from([$('.chapter__inset', ch), $('.chapter__line', ch)].filter(Boolean), { y: 40, opacity: 0, duration: 1, stagger: .1, ease: 'power3.out', scrollTrigger: { trigger: ch, containerAnimation: move, start: 'left 55%' } });
    });
  });
  mm.add(MOB, () => {
  ...
  const PP = {
    desk: {
      s: [[-48, -8, -8, .7], [-10, -46, -5, .68], [18, -48, 7, .64], [-34, -44, 6, .68], [48, 6, 7, .7], [-12, 46, -7, .64], [-36, 44, 5, .68], [36, 45, -6, .68], [38, -42, -7, .64], [-48, 40, 5, .68], [49, -38, 6, .64], [14, 47, 6, .68]],
      e: [[-43, -3, -3], [-12, -34, -2], [12, -35, 3], [-28, -36, 4], [43, 3, 3], [-8, 35, -3], [-26, 34, 2], [28, 36, -3], [32, -34, -4], [-42, 32, -3], [42, -32, 3], [10, 35, 2]]
    },
    mob: {
      s: [[-44, -42, -8, .7], [40, -46, 7, .7], [46, -28, -8, .66], [4, -44, 6, .7], [40, 42, 8, .7], [-46, 30, -7, .66], [-32, 46, 6, .7], [-2, 44, -6, .7]],
      e: [[-36, -35, -3], [36, -36, 3], [38, -20, -4], [0, -38, 2], [36, 36, 3], [-36, 35, -3], [-20, 37, 2], [0, 38, -2]]
    }
  };
  mm.add({ desk: DESK, mob: MOB }, ctx => {
    ...
    const title = $('.people__title'), clearTitle = () => (title.offsetWidth / 2 / innerWidth * 100) + 11;
    items.forEach((el, i) => {
  ...
  gsap.timeline({ scrollTrigger: { trigger: '.finale', start: 'top top', end: '+=210%', pin: true, scrub: 1, invalidateOnRefresh: true } })
    .fromTo('.finale__media img', { scale: 1.3 }, { scale: 1.05, ease: 'none', duration: 1 }, 0)
    .fromTo(credits, { y: 0 }, { y: () => -(credits.offsetHeight + innerHeight * 1.1), ease: 'none', duration: .65 }, 0)
    .fromTo('.finale__end > *', { autoAlpha: 0, y: 50 }, { autoAlpha: 1, y: 0, stagger: .04, duration: .22, ease: 'power2.out' }, .58);
